...
垂直连续模式隐藏底部横向缩略图列表，只保留紧凑页码入口；右侧轨道按整本漫画进度为每一页保留位置，以单个绘制层组成连续缩略图带。轨道高度由页数和紧凑步距动态计算，并限制在约半屏以内，不能直接铺满可用高度；iPad 使用稍大的步距和焦点缩略图，但仍遵守相同高度上限。普通项尺寸必须小于当前页密度对应的自然间距，只作为低遮挡刻度；手指所在的原 item 直接放大并向左突出，前后两页按距离逐级形变，禁止再叠一套独立焦点卡片。

放大区根据同一组 item 形变后的实际高度计算间距，把焦点上方的列表整体上推、下方列表整体下推；轨道首尾留白必须覆盖完整放大位移，长漫画也不能裁切或层叠邻页。缩略图带只读取已有 reader 预览缓存，整本缓存扫描与缩放必须离开主线程，只把最终结果合并回 UI；焦点缩略图也使用小目标尺寸，并经过短延迟后才允许加载。拖动只更新焦点和页码，松手时提交一次跳页，不能在快速扫过轨道时连续触发远端读取、解压或图片解码。
