...
- 资料库状态数据库：`Application Support/JamReader/AppLibraryV2.sqlite`
- 封面与派生资源：`Application Support/JamReader/LibraryAssets/<libraryID>/`
- 外部目录只作为内容源，不再承载 `.jamreaderlibrary`、`library.ydb` 或桌面兼容模式语义

## 1. 产品定位

这是一个面向 iPhone / iPad 的漫画阅读应用，整体方向可以概括为：

- 基于 JamReader 原版能力做 iOS 原生重写。
- 支持本地漫画库与远程漫画源并行使用。
...
- `JamReader/App/AppDependencies.swift`
- `JamReader/Features/Libraries/LibraryHomeView.swift`
- `JamReader/Features/Libraries/LibraryListViewModel.swift`
- `JamReader/Features/Browser/LibraryBrowserView.swift`
- `JamReader/Features/Browser/LibraryBrowserViewModel.swift`
- `JamReader/Features/Browser/LibrarySpecialCollectionView.swift`
- `JamReader/Features/Browser/LibraryOrganizationView.swift`
- `JamReader/Features/Reader/ComicReaderView.swift`
